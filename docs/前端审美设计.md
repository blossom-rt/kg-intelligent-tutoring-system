...
- 返回按钮（`ArrowLeft` 图标，触发 `goBack()`：优先 `router.back()`；无历史时按角色跳回 `/student` / `/teacher` / `/admin`）
- 页面标题 + 副标题
- 右侧操作插槽 `#actions`
...
│   ├── Profile.vue              # 个人信息（修改密码/资料）
│   ├── student/                 # 学生端子页面
│   │   ├── GraphView.vue        # 知识图谱浏览
│   │   ├── PathList.vue         # 学习路径列表
│   │   ├── PathDetail.vue       # 路径详情
│   │   ├── Practice.vue         # 知识点练习
│   │   ├── NodeStudy.vue        # 知识点学习（含学习资料）
│   │   ├── WrongBook.vue        # 错题本
│   │   ├── ExamList.vue         # 我的测评
│   │   ├── ExamTake.vue         # 测评作答
│   │   ├── ExamResult.vue       # 测评结果
│   │   ├── ThemeList.vue        # 跨学科主题列表
│   │   └── ThemeDetail.vue      # 主题详情
│   ├── teacher/                 # 教师端子页面
│   │   ├── CourseManage.vue / NodeManage.vue / QuestionManage.vue
│   │   ├── GraphEdit.vue / ExamManage.vue / ThemeManage.vue
│   │   ├── ClassAnalysis.vue / StudentPathList.vue
│   └── admin/                   # 管理员端子页面
│       ├── UserManage.vue / RoleManage.vue / CourseManage.vue
│       ├── NoticeManage.vue / OperLog.vue / AiLog.vue
├── components/
│   ├── AnimatedCharacters.vue   # 登录页四色角色插画
│   ├── StudentHeader.vue        # 统一页面头部（带返回按钮，goBack 复合逻辑）
│   ├── Sidebar.vue              # 学生/教师端侧边导航（可折叠，useSidebar.js）
│   ├── StudyPet.vue             # 右下角学习宠物（答对庆祝/答错安慰）
│   └── ThemeToggle.vue          # 暗色模式切换开关
├── styles/
│   └── global.css               # 全局设计系统 + Element Plus 覆盖（313 行）
├── api/                         # API 函数（student.js, teacher.js, admin.js, knowledge.js, user.js）
├── utils/
│   └── request.js               # Axios 实例 + 拦截器（baseURL=/api/kg）
└── router/
    └── index.js                 # 路由表
```
